...
function assignProductIds(products) {
  products.forEach((product) => {
    const productId = PRODUCT_ID_BY_TITLE[product.title];
    product.id = productId;

    if (product.card && productId) {
      product.card.dataset.productId = `${productId}`;
    }

    if (productId) {
      product.card
        ?.querySelectorAll('a.btn.btn-primary, button.btn.btn-primary')
        .forEach((button) => {
